...
const SATOSHI_NOTES = {
  ...
  '12cbQLTFMXRnSzktFkuoG3eHoMeFtpTu3S': 'First user-to-user Bitcoin transaction (to Hal Finney)'
};
...
async function checkAddressConnection(address) {
  try {
    ...
    // Check if this address is already known to be tinted
    try {
      const tintedInfo = await db.get(`tinted:${address}`);
      return {
        isConnected: true,
        isSatoshiAddress: false,
        degree: tintedInfo.degree,
        connectionPath: tintedInfo.path,
        transactions: await Promise.all(tintedInfo.path.map(async p => {
          try {
      ...
      // Not in tinted database, return no connection
      return {
        isConnected: false,
        isSatoshiAddress: false,
        degree: 0,
