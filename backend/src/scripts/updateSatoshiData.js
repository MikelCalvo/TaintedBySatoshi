...
async function processAddress(address, currentDegree, db) {
  ...
  try {
    ...
    for (const tx of transactions) {
      const isOutgoing = tx.inputs.some(input => input.prev_out.addr === address);
      ...
      if (isOutgoing) {
        ...
        const outputPromises = tx.out
          .filter(output => output.addr !== address && !SATOSHI_ADDRESSES.includes(output.addr))
          .map(async output => {
            try {
              // Check if we already have a shorter path
              try {
                const existing = await db.get(`tinted:${output.addr}`);
                if (existing.degree <= currentDegree) {
                  return null; // Skip if we already have a shorter or equal path
                }
              } catch (err) {
                // Address not yet tinted, proceed with new path
              }

              const parentTinting = await db.get(`tinted:${address}`);
              const path = [...parentTinting.path, {
                from: address,
                to: output.addr,
                txHash: tx.hash,
                amount: output.value
              ...
              await db.put(`tinted:${output.addr}`, {
                txHash: tx.hash,
              ...
              console.error(`Error processing output address ${output.addr}:`, err);
              return null;
            }
          });
...
async function updateSatoshiTransactions() {
  ...
  console.log(`Processing ${SATOSHI_ADDRESSES.length} known Satoshi addresses...`);
    ...
    for (let i = 0; i < SATOSHI_ADDRESSES.length; i += BATCH_SIZE) {
      ...
      for (const address of batch) {
        ...
        for (const tx of transactions) {
          ...
          if (isOutgoing) {
            // Store first-degree tinted addresses
            const outputPromises = tx.out
              .filter(output => output.addr !== address && !SATOSHI_ADDRESSES.includes(output.addr))
              .map(async output => {
                await db.put(`tinted:${output.addr}`, {
            ...
            const tintedAddresses = await Promise.all(outputPromises);
            
            // Queue for next degree processing
            for (const addr of tintedAddresses) {
