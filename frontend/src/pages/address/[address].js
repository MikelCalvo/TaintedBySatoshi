    ...
    if (!response.ok) {
      return {
        props: {
          ...
          error: {
            title: response.status === 429 ? 'Rate Limit Exceeded' : 'Error Checking Address',
...
const getDegreeDescription = (degree) => {
  ...
  if (degree === 2) return "Received from an address that received from Satoshi";
...
export default function AddressPage({ address, result, error }) {
  ...
  return (
    <>
      <Head>
        <title>{address} - Tinted By Satoshi</title>
        <meta name="description" content={`Check if Bitcoin address ${address} has any connection to Satoshi Nakamoto's wallets`} />
      </Head>
              ...
              <Stack spacing={4}>
                {result.isSatoshiAddress ? (
                  <>
                    ...
                    <Typography>
                      This is one of Satoshi Nakamoto's known addresses. {result.note}
                    </Typography>
                  </>
                ) : (
                  <>
                    <Box>
                      <Typography variant="h5" gutterBottom>
                        ...
                        <Typography 
                          component="span" 
                          color={result.isConnected ? 'success.main' : 'error.main'}
                        >
                          {result.isConnected ? 'Connected to Satoshi' : 'No connection found'}
                        </Typography>
                      </Typography>
                      {result.isConnected && (
